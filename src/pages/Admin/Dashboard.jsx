...
import { firestore, doc, getDoc, collection, getDocs, setDoc } from "../../firebase/firebase";
            ...
            <div className="container mt-4">
                <div className="row">
                    <div className="col chart-card">
                        ...
                        <h1 className="text-center">${totalRevenue}</h1>
                    </div>
                    <div className="col chart-card">
                        <p>Total Sold Ticket Count</p>
                        <i className="bi bi-ticket"></i>
                        <h1 className="text-center">{totalSoldTickets}</h1>
                    </div>
                    <div className="col chart-card">
                        <p>Vision Movie Count</p>
                        <i className="bi bi-film"></i>
                        <h1 className="text-center">{visionMovieCount}</h1>
                    </div>
                    <form action="" className="form-container changeCarousel">
                        <p>Change homepage carousel images.(All url's should not be empty!)</p>
                        {images.map((img, index) => (
                            <div key={index}>
                                <label htmlFor={`img${index}`}>Image {index + 1} URL</label>
                                <input
                                    type="text"
                                    name={`img${index}`}
                        ...
                        <button type="submit" className="btn btn-dark save-button" onClick={addCarousel}>Save</button>
                    </form>
                </div>
            </div>
        </div>
